...
/***/ function(module, exports, __webpack_require__) {
	...
	function TimekitBooking() {
	  ...
	  var LVSFetchCalendarSlots = function() {
	    var google_events = [];
	    for (var i = window.g_cal_events.length - 1; i >= 0; i--) {
	    	var start = moment(window.g_cal_events[i].start)._i;
	    	var end = moment(window.g_cal_events[i].end)._i;
	    	var event_object = {
	    		start: start,
	    		end: end
	    	}

	    	google_events.unshift(event_object);
	    }
		return google_events;
	  };

	  var LVSSetupSlotFilters = function(events) {
	  	var filters_object = {
	    	filters: {
	            or: [
	            ]
	    ...
	    for (var i = events.length - 1; i >= 0; i--) {
	    	filters_object.filters.or.push({ "between_timestamps": {"start": events[i].start, "end": events[i].end, "timezone": moment.tz.guess()}});
	    }
	  ...
	  var timekitFindTime = function() {
		...
		var cal_slots_data = LVSFetchCalendarSlots();
		var new_filters = LVSSetupSlotFilters(cal_slots_data);
	    $.extend(args, new_filters);
